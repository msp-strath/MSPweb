...
instance FromJSON NewsDate where
  parseJSON = withObject "NewsDate" $ \ v -> do
    single v <|> range v
    where
      single v = do
...
instance FromJSON News where
  parseJSON = withObject "News" $ \ v -> event v <|> news v
    where
      event v = do
        date <- parseJSON (Object v)
        name <- v .: "event"
        url <- v .: "url"
        more <- v .:? "description"
        pure (Event date name url more)
      news v = do
        desc <- v .: "description"
        date <- parseJSON (Object v)
        pure (News date desc)
...
newsToHTML (News date desc) = ddt (newsDateToHTML date, desc)
newsToHTML (Event date name url more) = ddt (newsDateToHTML date, desc)
 where
   desc = "We are organising the <a href=\"" ++ url ++ "\">" ++ name ++ "</a>." ++ maybe "" (" " ++) more

eventToHTML :: News -> HTML
eventToHTML(Event date name url _) = ddt (newsDateToHTML date, anchor url name)
...
generateNews file = do
  ...
  case decodeEither' f of
    Left err ->
      error (show err)
    Right news' -> do
      let news = sortOn (Down . date) news'
      ...
      let events = filter isEvent news
      pure (dlist new, map (\ x -> (startYear (head x), dlist x)) oldGrouped, tag "dl" (unlines (map eventToHTML events)))
    where
...
main = do
  ...
  let headerEvents = "### default.html(section.events=current)\n<!-- DO NOT EDIT THIS FILE DIRECTLY — EDIT _news.yaml AND RUN GenerateNews.hs INSTEAD -->"
  ...
  (newNews, oldNews, events) <- generateNews "_news.yaml"
  ...
  writeFile "events.html" (headerEvents ++ h2 "Events" ++ "<p>We have organised various events over the years.</p>" ++ events)
